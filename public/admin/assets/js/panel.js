          ...
          if (res.success == true) {
            $.each(res.data, function (x, y) {
              ...
              if ($selectField.attr("multiple")) {
                ...
                // Menghapus tanda kurung siku dari string
                // const cleanedString = y.replace(/[\[\]\"\\]/g, "");
              ...
              if (x == "anggaran" || x == "harga_satuan") {
                let value = y;
                if (value !== "") {
                  // Pastikan bahwa 'value' adalah angka
                  if (typeof value === 'string') {
                    // Jika 'value' adalah string, konversi menjadi angka terlebih dahulu
                    value = parseFloat(value);
                  }

                  // Format angka menjadi Rupiah menggunakan numeral.js
                  value = numeral(value).format('0,0');

                  // Atur nilai pada elemen yang memiliki nama 'anggaran'
                  $("input[name='" + x + "']").val('Rp ' + value);
                }
              }
            });
          }
